...
import { ChevronDown, GripVertical, Plus, Check, Search, Workflow, X } from "lucide-react";
import { cn } from "@/lib/utils";
import { useTemplates } from "./templates/store";
import type { Card, ModuleKey } from "./templates/types";
 ...
 * - Card list grouped by `card.group` with collapsible sections
 * - Header shows total selected; each group header shows selected/total
 * - Panel height adapts to viewport; long content scrolls
 */
  ...
  return (
    <aside
      className={cn(
        ...
        "max-h-[calc(100vh-8rem)] shadow-[0_20px_60px_-25px_oklch(0.62_0.22_295/0.35),0_4px_12px_-6px_rgba(0,0,0,0.08)] ring-1 ring-black/[0.04]",
      )}
    >
      {/* Header */}
        ...
        {groups.length === 0 ? (
          ...
          <div className="space-y-2">
            {groups.map(([groupName, groupCards]) => {
              ...
              const groupSelected = groupCards.filter((c) => c.enabled).length;
              return (
                <div key={groupName}>
                  <button
                    type="button"
                    onClick={() => toggleGroup(groupName)}
                    className="flex w-full items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-left transition-colors hover:bg-muted/60"
                  >
                    <ChevronDown
                      className={cn(
                    ...
                    />
                    <span className="flex-1 truncate text-[12px] font-semibold tracking-tight text-foreground/80">
                      {groupName}
                    </span>
                    <span className="text-[11px] tabular-nums text-muted-foreground">
                      <span className="font-medium text-primary">{groupSelected}</span>
